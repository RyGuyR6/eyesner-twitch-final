...
PROJECT_ROOT="$(pwd)"
BACKUP_DIR=".premium-backup-$(date +%Y%m%d-%H%M%S)"

echo "=========================================="
echo " EYESNER PREMIUM STREAM PACKAGE UPGRADER"
echo "=========================================="
echo
...
# ------------------------------------------------------------
# Remotion compositions: 1920x1080 at 60 FPS
# ------------------------------------------------------------
...
cat > scripts/render-scenes.ts <<'EOF'
...
# ------------------------------------------------------------
# OBS package generator
# ------------------------------------------------------------

cat > scripts/generate-obs-package.ts <<'EOF'
import fs from 'fs-extra';
import path from 'node:path';
import {execFileSync} from 'node:child_process';
...
# ------------------------------------------------------------
# Ensure the package scripts are present
# ------------------------------------------------------------
...
packageJson.scripts['render:scenes'] =
  'tsx scripts/render-scenes.ts';

packageJson.scripts['obs:package'] =
  'tsx scripts/generate-obs-package.ts';

packageJson.scripts['build:premium'] =
  'npm run render:scenes && npm run obs:package';

packageJson.scripts['verify:premium'] =
  'for f in dist/renders/*; do echo "===== $f ====="; ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,width,height,r_frame_rate,pix_fmt -show_entries format=duration,size,bit_rate -of default=noprint_wrappers=1 "$f"; done';
...
if npm run typecheck --if-present; then
  ...
  echo "Your original files remain in: $BACKUP_DIR"
  exit 1
fi

echo
echo "Installing zip utility if needed..."
...
echo
echo "Building premium delivery package..."
npm run build:premium

echo
echo "Verifying all rendered files..."

FAILED=0

for file in dist/renders/*; do
  [[ -f "$file" ]] || continue
  ...
  if [[ "$WIDTH" != "1920" ]]; then
    echo "ERROR: Incorrect width."
    FAILED=1
  fi

  if [[ "$HEIGHT" != "1080" ]]; then
    echo "ERROR: Incorrect height."
    FAILED=1
  fi

  if [[ "$FPS" != "60/1" ]]; then
    echo "ERROR: Incorrect frame rate."
    FAILED=1
  fi

  if [[ "$file" == *.mp4 && "$CODEC" != "h264" ]]; then
    echo "ERROR: MP4 file is not H.264."
    FAILED=1
  fi

  if [[ "$file" == *.webm && "$CODEC" != "vp9" ]]; then
    echo "ERROR: WebM file is not VP9."
    FAILED=1
  fi
done
...
if [[ "$FAILED" -ne 0 ]]; then
  echo "One or more exports failed verification."
  exit 1
fi

echo "=========================================="
echo " PREMIUM PACKAGE COMPLETE"
echo "=========================================="
echo
echo "✓ 1920x1080 delivery resolution"
echo "✓ 60 FPS"
echo "✓ Premium H.264 full-screen scenes"
echo "✓ Transparent VP9 overlays and alerts"
echo "✓ OBS scene collection"
echo "✓ Windows one-click installer"
echo "✓ Uninstaller"
echo "✓ Media verification report"
echo "✓ ZIP delivery package"
echo
echo "Final package:"
echo "dist/delivery/Eyesner-Premium-Twitch-OBS-Package.zip"
echo
echo "Backup:"
echo "$BACKUP_DIR"
